...
// Bandera para evitar la re-creación en recargas en caliente (hot-reloads)
...
async function setup() {
  ...
  // Solo se ejecuta una vez por inicio del servidor
  if (!dbInitialized) {
    // Eliminar tablas para asegurar un esquema limpio en cada reinicio del desarrollo
    await db.exec(`DROP TABLE IF EXISTS passwords;`);
  ...
  // Verificar si las tablas están vacías antes de sembrar los datos
